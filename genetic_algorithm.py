...
def runGA(generations, populationSize, mutationRate, GAPoses):
    ...
    1. We create a list where the best chromosome for each frame generated 
        by our GA will be stored (generatedChromosomeList).
    2. Use several nested loops to ensure that the Genetic Algorithm runs for the allocated time.
    3. We store everything in a for loop that loops throughout the whole length of the target chromosome list. 
        This ensures that we run a loop for every frame in the animation.
    4. Then we enter the next for loop which will loop for the desired ampunt of generations specified by the user. 
        Inside this loop, we calculate the best two parents in the current generations population with calculateBestFitness.
    5.  We then pass the generations 2 best parents into createNewPopulation, where the new population for the next generation of the GA are created.    
    6. We then call the mutate function to mutate this new generation and append it to a new list called mutatedPop. Finally, we set the value of population to the value of mutatedPop. 
        This assigns the next generations population to the variable. This will loop for the total number of generations until the best chromosome is found. This best chromosome across 
        all the generations is then appended to the generatedChromosomeList. 
    7.  Once all of this has looped 300 times we have very frame for our walk cycle.

    Mutation rate:
    1. After that we perform mutation on the new chromosome to give further diversity. This function takes the chromosome and a mutation rate determined by the user as parameters.  
    ...
    Args:
        ...
        populationSize: Size of the population for each generation
        mutationRate: Mutation rate for the GA
        GAPoses: List of target chromosomes generated by the main program
        targetChromosome: The current target chromosome to match
        bestFitness: The best fitness found so far
        bestChromosome: The best chromosome found so far
        genBestIndex: Index of the best chromosome in the current generation
        secondBestChromosomeIndex: Index of the second best chromosome in the current generation
        genBestFitness: Best fitness in the current generation
        generatedChromosomeList: List of best chromosomes for each target pose
    """
